...
function ProviderChooser({
  ...
  const options: OptionWithDescription<ProviderChoice>[] = [
    ...
    },
    {
      ...
      value: 'codex',
      description:
        'Use ChatGPT via official Codex auth. If needed, sign in with /onboard-chatgpt first',
    },
  ]
...
function resolveCodexCredentials(processEnv: NodeJS.ProcessEnv):
  ...
  if (!credentials.apiKey) {
    ...
    return {
      ok: false,
      message: `Codex setup needs existing credentials. Run /${getCommandName(onboardChatGPT)} to sign in with the browser, or set CODEX_API_KEY manually. ${authHint}`,
    }
  }
  ...
  if (!credentials.accountId) {
    return {
      ok: false,
      message:
        `Codex auth is missing chatgpt_account_id. Run /${getCommandName(onboardChatGPT)} or set CHATGPT_ACCOUNT_ID/CODEX_ACCOUNT_ID first.`,
    }
  }
